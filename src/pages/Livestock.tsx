...
import { Beef, ShoppingCart, ArrowLeft } from "lucide-react";

const Livestock = () => {
  const [searchTerm, setSearchTerm] = useState("");
  const { products, loading, refreshProducts } = useProducts();
  // Only show products of type 'livestock'
  const livestockProducts = products.filter(p => p.product_type === 'livestock');
  ...
  const handleAddToCart = (product: {
    ...
    addToCart({
      ...
      image: product.image_url || "https://images.unsplash.com/photo-1597024436273-b920b9ff5bcc?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&q=80",
      ...
      <div className="py-16">
        <div className="container mx-auto px-4">
          <div className="text-center">
            <p>Loading livestock products...</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="container mx-auto px-4">
        <div className="mb-12">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="flex items-center gap-2">
              <Beef className="w-8 h-8 text-agro-green" />
              <h1 className="text-agro-green-dark">Livestock Products</h1>
            </div>
          </div>
          <div className="flex justify-between items-center">
            <p className="text-gray-600 max-w-3xl">
              Discover premium livestock from verified farmers. All animals are healthy, well-cared for, and ready for purchase.
            </p>
            <Button 
          ...
          {filteredProducts.map((product) => (
            <Card key={product.id} className="overflow-hidden hover:shadow-lg transition-shadow duration-300 group h-full flex flex-col">
              <div className="h-48 overflow-hidden">
                <img 
                  src={product.image_url || "https://images.unsplash.com/photo-1597024436273-b920b9ff5bcc?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&q=80"} 
                  alt={product.name} 
        ...
        {filteredProducts.length === 0 && (
          <div className="text-center py-12">
            <Beef className="w-16 h-16 text-gray-400 mx-auto mb-4" />
